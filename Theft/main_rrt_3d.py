...
#check if newly added sample is in the free configuration space
	def isfree(self):
		...
		if x<=0 or y<=0 or z<=0 or z==999 or y==999:
			G.remove_node(n)
			return 0
...
#path to goal
	...
	def prun(self):	
		...
		for e in range(len(self.path)-1):
			...
			if E.inobstacle(x1,y1,z1,x2,y2,z2)==0: #CC is detected
				c=0
